...
def split_segments(args):
    ...
    # Match the reference name in the segment line. Example: SN:Z:GRCh38#0#chr1 - Alternatively: GRCh38.chr1
...
def convertConversionCodeSingle(args):
    # This will take in a single value conversion code and convert it to a tab delimited string of TRUE or FALSE values. Like those taken in by the convertFlagsToFlagCode function.
    # Like so: convertFlagsToFlagCode(isReferenceFlag, hasAnchorFlag, lengthFlag, lengthMatchFlag, hasMultipleAnchorsFlag, isQueryFlag, generalErrorFlag)
    # Using this code:
    # flagCode = 0
    # if isReferenceFlag == True:
    #     flagCode += 1
...
def convertConversionCodes(args):
    # This will take in a .methylC file generated by Ixchel and convert the conversion codes to a tab delimited string of TRUE or FALSE values. Like those taken in by the convertFlagsToFlagCode function.
    # Like so: convertFlagsToFlagCode(isReferenceFlag, hasAnchorFlag, lengthFlag, lengthMatchFlag, hasMultipleAnchorsFlag, isQueryFlag, generalErrorFlag)
    # The output should be chromosome, start, stop, conversionCode, isReferenceFlag, hasAnchorFlag, lengthFlag, lengthMatchFlag, hasMultipleAnchorsFlag, isQueryFlag, generalErrorFlag
    # This will be useful for the user to understand what the conversion code means.
    # Load in the .methylC file
    methylCfile = args.input
    # Check to make sure the .methylC file exists
    if not os.path.exists(methylCfile):
        print(f"Error: {methylCfile} does not exist.")
        sys.exit(1)
    # Generate the output file name. Strip the extension from the input file and add .conversioncodes
    outputfile = os.path.splitext(methylCfile)[0] + ".conversioncodes"
    # Loop through the .methylC file and convert the conversion codes to a tab delimited string of TRUE or FALSE values
...
def main():
    ...
    # Convert specific conversion code from Ixchel generated .methylC file
    # corresponds to 9th column in .methylC file
    # These are generated by the convertFlagsToFlagCode function
